...
function createRequest(options: { limit?: string; authorization?: string } = {}): Request {
  const url = new URL('http://localhost/api/popups/featured');
    ...
    items: [
      {
        ...
        liked: true,
        stats: { likeCount: 320, viewCount: 1850 },
        overlay: null,
        phase: {
          type: 'DRAW',
          status: 'OPEN',
      ...
      },
      {
        ...
        liked: false,
        stats: { likeCount: 210, viewCount: 1420 },
        overlay: null,
        phase: {
          type: 'DRAW',
          status: 'OPEN',
...
describe('GET /api/popups/featured', () => {
  ...
  describe('limit 검증', () => {
    it('limit 파라미터가 없으면 기본값 10으로 백엔드를 호출한다', async () => {
      const mockFetch = jest
        .fn()
        .mockResolvedValue(createMockBackendResponse(MOCK_FEATURED_RESPONSE));
  ...
  describe('Authorization 헤더', () => {
    ...
    it('Authorization 헤더가 있으면 백엔드에 그대로 전달한다', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining('/popups/featured'),
  ...
  describe('백엔드 프록시', () => {
    it('백엔드가 200이면 응답을 그대로 통과한다', async () => {
      global.fetch = jest
        .fn()
        .mockResolvedValue(createMockBackendResponse(MOCK_FEATURED_RESPONSE, 200));

      const response = await GET(createRequest());
      const body = await response.json();

      expect(response.status).toBe(200);
      expect(body).toEqual(MOCK_FEATURED_RESPONSE);
    });
